...
/**
 * Garis Panduan - PDF uploads for guidelines.
 * Uses FileDropZone with 2MB max, PDF only.
 */
const GarisPanduan = () => {
  ...
  const handleFiles = (fileList) => {
    ...
    const valid = list.filter((f) => f.size <= MAX_SIZE && f.type === 'application/pdf');
    if (valid.length < list.length) {
      setUploadError('Hanya fail PDF sehingga 2MB dibenarkan.');
    } else {
      setUploadError(null);
    }
    setFiles(valid);
  ...
  return (
    <PageLayout title="Garis Panduan">
      <Card className="p-6">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <FileDropZone
              ...
              error={uploadError}
              hint="Seret fail PDF ke sini atau klik untuk pilih. Maksimum 2MB."
            />
            {files.length > 0 && (
