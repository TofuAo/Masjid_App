...
CREATE TABLE IF NOT EXISTS achievements (
    ...
    requirement_type VARCHAR(50), -- e.g., 'attendance_count', 'exam_score', 'streak_days'
...
CREATE TABLE IF NOT EXISTS points_history (
    ...
    source_type VARCHAR(50), -- e.g., 'attendance', 'achievement', 'exam', 'payment'
    source_id INT, -- Reference to source record (e.g., attendance.id, achievement.id)
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (user_ic) REFERENCES users(ic) ON DELETE CASCADE,
...
-- 6. Leaderboard Cache (for performance)
CREATE TABLE IF NOT EXISTS leaderboard_cache (
    ...
    category VARCHAR(50) DEFAULT 'overall', -- 'overall', 'monthly', 'attendance', 'academic'
    period_start DATE,
    period_end DATE,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (user_ic) REFERENCES users(ic) ON DELETE CASCADE,
