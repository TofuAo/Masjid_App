...
// Run with: node scripts/generateMockData2025.js
...
async function generateMockData() {
  ...
  try {
    ...
    for (const student of students) {
      let sessions = [];
      try {
        if (typeof student.sessions === 'string') {
          sessions = JSON.parse(student.sessions);
        } else if (Array.isArray(student.sessions)) {
          sessions = student.sessions;
        } else {
          sessions = [];
        }
      } catch (e) {
        console.log(`Warning: Could not parse sessions for student ${student.ic}:`, e.message);
        sessions = [];
      }
      
      // Filter out non-string values
      sessions = sessions.filter(s => typeof s === 'string' && s.trim().length > 0);
      
      if (!Array.isArray(sessions) || sessions.length === 0) {
        console.log(`Skipping student ${student.ic} - no valid sessions`);
        continue;
      }
