...
// Complete staff list from the image with correct ICs (NO K/P)
const staffList = [
...
const updateICByName = async () => {
  const connection = await pool.getConnection();
  
  try {
    console.log('='.repeat(80));
    console.log('UPDATING IC NUMBERS BY NAME MATCHING');
    console.log('Matching users by name and updating to correct IC from image');
    console.log('='.repeat(80));
    ...
    for (const staff of staffList) {
      try {
        console.log(`\n[${staff.name}]`);
        console.log(`  Correct IC from image: ${staff.correctIC}`);
        ...
        const matches = allUsers.filter(user => {
          ...
          // Check if most words match (at least 3 words)
        ...
        if (matches.length > 1) {
          console.log(`  ⚠️  Multiple matches found (${matches.length}), updating all:`);
          matches.forEach((u, idx) => {
            console.log(`     ${idx + 1}. ${u.nama} (Current IC: ${u.ic})`);
          });
        }
        ...
        for (const user of matches) {
          ...
          const newICNorm = String(newIC).replace(/\D/g, '');
          if (oldICNorm === newICNorm) {
            console.log(`  ✅ Already correct`);
            results.alreadyCorrect.push({ name: user.nama, ic: oldIC });
            continue;
          }
          ...
          if (existing.length > 0 && existing[0].ic !== oldIC) {
            ...
            console.log(`  Will delete old entry and keep the one with correct IC...`);
            ...
            results.updated.push({ name: user.nama, oldIC, newIC, action: 'deleted_old' });
          ...
          console.log(`  ✅ Updated: ${oldIC} → ${newIC}`);
          results.updated.push({ name: user.nama, oldIC, newIC });
        }
