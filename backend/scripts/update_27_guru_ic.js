...
const update27GuruIC = async () => {
  const connection = await pool.getConnection();
  
  try {
    console.log('='.repeat(80));
    console.log('UPDATING 27 GURU IC NUMBERS');
    console.log('Updating existing entries to have correct ICs from image');
    console.log('='.repeat(80));
    ...
    for (const staff of staffList) {
      try {
        ...
        // Check if new IC already exists for different user
        ...
        if (existing.length > 0 && existing[0].ic !== oldIC) {
          ...
          console.log(`  Updating that entry instead...`);
          ...
          await connection.execute(
            'UPDATE teachers SET user_ic = ?, kepakaran = ? WHERE user_ic = ?',
            [newIC, kepakaran, newIC]
          );
        ...
        // Update IC in users table (using temporary value to avoid constraint violation)
        console.log(`  Updating IC from ${oldIC} to ${newIC}...`);
        
        // Use a temporary IC first
        const tempIC = `TEMP_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
        
        // Step 1: Update to temporary IC
        await connection.execute('UPDATE users SET ic = ? WHERE ic = ?', [tempIC, oldIC]);
        
        // Step 2: Update teachers table
        await connection.execute('UPDATE teachers SET user_ic = ? WHERE user_ic = ?', [tempIC, oldIC]);
        
        // Step 3: Update classes
        await connection.execute('UPDATE classes SET guru_ic = ? WHERE guru_ic = ?', [tempIC, oldIC]);
        
        // Step 4: Update user_roles
        await connection.execute('UPDATE user_roles SET user_ic = ? WHERE user_ic = ?', [tempIC, oldIC]);
        
        // Step 5: Update to final correct IC
        await connection.execute('UPDATE users SET ic = ? WHERE ic = ?', [newIC, tempIC]);
        await connection.execute('UPDATE teachers SET user_ic = ? WHERE user_ic = ?', [newIC, tempIC]);
        await connection.execute('UPDATE classes SET guru_ic = ? WHERE guru_ic = ?', [newIC, tempIC]);
        await connection.execute('UPDATE user_roles SET user_ic = ? WHERE user_ic = ?', [newIC, tempIC]);
        
        // Update other related tables
        const relatedTables = ['attendance', 'results', 'fees', 'payments'];
        for (const table of relatedTables) {
          try {
            await connection.execute(`UPDATE ${table} SET user_ic = ? WHERE user_ic = ?`, [newIC, tempIC]);
          } catch (e) {
